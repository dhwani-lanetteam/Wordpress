...
function shopinpic_settings_page() {
	...
	if (isset($_POST['check_woo'])) {
		$wooSiteUrl = '';
		//{{{ 
		$jsonContents = curl_get_file_contents('http://shopinpic.com/imgMap3/checkWooData.php?apiKey='.get_option('shopinpic_apikey'));
...
return;
		...
		$jsonContents = curl_get_file_contents('http://shopinpic.com/imgMap3/getDatas.php?imageUrl=sample&apiKey='.$apiKey);
		$res = json_decode($jsonContents);
