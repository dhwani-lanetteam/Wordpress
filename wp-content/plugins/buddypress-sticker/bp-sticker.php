...
if ( !defined( 'BPST_PLUGIN_VERSION' ) )
	define( 'BPST_PLUGIN_VERSION', '1.4.0' );
...
function bp_sticker_ajax(){
    ...
    foreach (self::replace_with_st() as $codes => $imgs) {     
		 ...
		 $html.="<img src='$icon' data-code=':$filename:' class='bpsmiley' /> ";
        }	
	 $html .="</div>"; 
     echo  $html;
 	wp_die( );  
...
$images_dir = BPST_PLUGIN_DIR."images/sticons/";
if ($opendir = opendir($images_dir)) { 
$file_display = array(
    'jpg',
    'jpeg',
    'png',
  ...
  $file_type = strtolower(end(explode('.', $file)));
   ...
   if($file != "." && $file != ".."&& in_array($file_type, $file_display) == true) {
    $images["$file"]=$file;  
	 $times[] = filemtime($images_dir  . $file) . ',' . $file;
   }
    }
...
array_multisort($images, SORT_NUMERIC, $times);
   return $images;      
   }
